...
def append_new_entry_to_excel(path, new_row):
    ...
    ws = wb.active  # single sheet assumed

    # Columns header from first row
    headers = [cell.value for cell in ws[1]]

    # Align new_row to headers (fill missing columns if needed)
...
if submit and discrepancy_input and corrective_input:
    ...
    if not exact.empty:
        ...
        st.dataframe(exact[['Description', 'Corrective Action', 'Actual Historic Hours', 'Fair Quote (hrs)', 'Occurrences']].reset_index(drop=True))
    elif not top2.empty:
        row = top2.iloc[0]
        conclusion, style_class, percent_diff, diff_class = get_decision_conclusion(supplier_hours, row['Fair Quote (hrs)'])
        st.markdown(f"""
