...
if uploaded_file:
    with st.spinner("🔄 Processing your file..."):
        ...
        for i in range(1, 101):
            time.sleep(0.01)
            placeholder.markdown(f"<h4 style='text-align:center;'>Processing your file... {i}%</h4>", unsafe_allow_html=True)
        placeholder.empty()
        ...
        if submitted and (corrective_input and discrepancy_input):
            ...
            def calc_overlap(a, b):
                ...
                return 100 * len(a_set & b_set) / max(len(a_set), 1)

            def combine_overlap(x):
                d_overlap = calc_overlap(norm_disc, normalize_text(x.split('|')[0]))
                c_overlap = calc_overlap(norm_corr, normalize_text(x.split('|')[1]))
                return (d_overlap + c_overlap) / 2

            df['Overlap'] = df['Combined Key'].apply(combine_overlap)

            approx_match_df = df[df['Overlap'] >= 50].copy()
            below_approx_match_df = df[(df['Overlap'] < 50)].sort_values(by='Overlap', ascending=False).head(1)
            ...
            if not approx_match_df.empty:
                ...
                st.warning("No approximate matches found.")

            if not below_approx_match_df.empty:
                st.info("### Closest Reference Point (Use only as reference)")
                below_row = below_approx_match_df.iloc[0]
                row = format_row(below_row)
