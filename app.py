...
if uploaded_file:
    ...
    year_filter = st.sidebar.multiselect("Select Year(s):", sorted(df['Year'].dropna().unique()), default=None)
    ...
    st.dataframe(filtered_df[['Orig. Card #', 'Description', 'Corrective Action', 'Total Hours', 'Reference Hours', 'Fair Quote (hrs)', 'Occurrences']])
    ...
    download_excel = to_download.to_excel(index=False)
    st.download_button("Download Filtered Results", data=download_excel, file_name="Fair_Quote_Results.xlsx")
